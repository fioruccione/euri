...
**RETR-03 — accettazione live della riscrittura locale dopo riavvio manuale.**

La Fase 4 e' ora chiusa: rimozione della coppia collidente, riscrittura naturale
con RAG e riscrittura senza RAG sono tutte 2/2. Il resolver implementato passa
stato e `source_turn_ref` al Brain soltanto quando l'ultima coppia e' grounded;
raw, scope e segmenti restano invariati. Dopo il prossimo riavvio manuale,
ripetere il caso attuale/proposto e un caso generico non ICMA prima di chiudere.
...
| RETR-03 | P0 | implementato, regressioni generiche verdi, live aperto | Fase 4 con timeout: rimozione collisione 2/2, riscrittura naturale con RAG 2/2, senza RAG 2/2; runtime riscrive solo stato esplicito dell'ultima coppia grounded e conserva raw + `source_turn_ref` | dopo riavvio manuale ripetere attuale/proposto e un referente generico; verificare astensione senza stato e nessun crossing scope/segmento | referente locale corretto senza domanda inutile; raw invariato; nessuna riscrittura se lo stato non e' grounded; rollback sistemico verificabile |
| RETR-04 | P0 | implementato offline, 92/92 unit, live aperto | panoramica Regrado corretta ma 3 nodi sostanziali su 156 e GPU scherzosa entrata da insight interno; schema corrente contiene 18 fonti Regrado ma il focus composto `RegradoPP` era separato in `Regrado`+`PP` | dopo RETR-03 e riavvio, ripetere overview+timeline Regrado distinguendo certo/da verificare e ispezionare nodi/lineage; non correggere il dominio con una regola Regrado-specifica | bundle entity-first porta fonti dirette diversificate e aggiornamenti recenti; insight tentativi fuori dalla ricostruzione fattuale; nessun trasferimento tramite acronimo nudo |
| DOC-01 | P1 | implementato offline, 92/92 unit, prima live fail-closed, secondo fix da riavviare | il caso Yizumi/CEN iniziale mostrava upload 2/2 ma lettura 1/2; dopo il primo fix la live ha estratto entrambi i nuovi PDF ma ha incluso anche il vecchio `izumi.pdf` per match dello stem, rilevando correttamente 3 file/2 hash e rifiutando il confronto | dopo riavvio ripetere senza pulire la coda: i due filename completi timestampati devono escludere la vecchia copia; poi provare un file volutamente illeggibile | UI mostra ricevuta 2/2 con nomi e hash distinti; risposta attribuisce ogni dato alla fonte; su 1/2 o duplicato reale nessun confronto, operazione fallita e nessuna dichiarazione di lettura priva di tool |
...
| CORR-01 | P0 | implementato, riparazione organica applicata, nuovo live aperto | resolver bounded e link atomico verificati; il caso GPU/Regrado ha mostrato che il signal legacy vedeva solo memory ID e perdeva l'insight esposto; ora conserva anche manifest tipizzato e `candidate_derived_ids` senza mutare automaticamente il derivato | al prossimo avvio ripetere una correzione+save nuova in scope controllato e verificare memory/insight distinti nel signal; nessuna generalizzazione a cambi di opinione | relazione `correction_of`/`superseded_by` e signal `resolved` osservati; provenienza tipizzata della risposta; nessun duplicato passivo, falsa dichiarazione di commit o supersede estraneo |
| CORR-03 | P0 | implementato offline, replay organico read-only verde, live in coda | le proposte `proposal_only` hanno ora un consumer owner condiviso fra voce/UI, lease per scope, target sotto provenienza+hash, backoff e quarantena dei link falliti; il primo replay ha falsificato il KNN libero, il secondo ha escluso 5/5 target non provati lasciando signal e lease invariati; solo i nuovi signal marcati `owner_review_contract_version=1` sono reclamabili dal runtime | dopo l'accettazione RETR-03 e un riavvio, creare un nuovo signal controllato; verificare domanda singola, scelta esplicita e commit oppure astensione; i cinque casi storici non versionati restano esclusi dalla coda | una sola domanda fra i canali; zero mutazioni pre-consenso; target estranei non applicabili; `APPLY` atomico o `repair_required`; `SEPARATE` salva solo la nuova frase owner esatta |
