...
| SEM-01 | P2 | rollout osservazionale | frame semantico unico attivo su voce/Mobile/Silent Chat; live: alias esplicita `Joe Style→Gio Style`, bootstrap owner senza wake e risposta≠azione riusciti. Il secondo pass ha aggiunto fatti v3 con modalità/durata e proiezione canonica `current_turn`, dopo i casi MFI `ephemeral` e `Geostyle` non propagato; 70/70 unit verdi | osservare casi organici di recall/azione, addressedness, proiezioni canoniche e `candidate/ephemeral/no_store`; verificare che prove tecniche riutilizzabili entrino nel passivo preservando incertezza. Nessuna regex su nomi o frasi | nessuna azione contestuale priva di effetto grounded, nessuna alias implicita persistente e nessuna memoria effimera nei pass osservati, senza perdere fatti riutilizzabili o richieste operative vere |
